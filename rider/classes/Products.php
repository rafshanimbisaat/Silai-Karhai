...
class Products
{
	...
	public function addCategory($name){
		$q = $this->con->query("SELECT * FROM rider_info WHERE first_name = '$name' LIMIT 1");
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Category already exists'];
		}else{
			$q = $this->con->query("INSERT INTO rider_info (first_name,last_name,email,password,mobile,address1,CNIC,BIKE_NO,Area) VALUES ('$name')");
			if ($q) {
	...
	public function getCategories(){
		$q = $this->con->query("SELECT * FROM rider_infoo");
		$ar = [];
		if ($q->num_rows > 0) {
			while ($row = $q->fetch_assoc()) {
				$ar[] = $row;
